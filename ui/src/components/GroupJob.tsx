...
const GroupJob: React.FC<GroupJobProps> = ({ setCurrentPage }) => {
  ...
  const [confirmation, setConfirmation] = useState<ConfirmationMessage>({
    message: null,
    type: "",
  });

  useEffect(() => {
    fetchNui("getGroupJobSteps").then((data: unknown) => {
      // Assert the type of data to GroupJobStep[]
      const steps = data as GroupJobStep[];
      if (steps && steps.length > 0) {
        setGroupJobSteps(steps);
      }
    });
  }, []);
          ...
          <div className="relative border-l border-accent ml-2 pl-2">
            {groupJobSteps &&
              groupJobSteps.map((step, index) => (
                <div key={index} className="mb-6 flex items-center text-left">
